...
## Notes
- Timing from pynq-helloworld notebooks (7 runs each, timeit)
- Power measured via INA260 at `/sys/class/hwmon/hwmon2/power1_input`
- CPU uses PIL (Python Imaging Library) BICUBIC resize
- FPGA uses AXI DMA + custom resize IP from pynq-helloworld overlay

## How to Reproduce
```bash
# Open Jupyter at http://<board-ip>:9090/lab  password: xilinx
# Run: pynq-helloworld/resizer_ps.ipynb  (CPU)
# Run: pynq-helloworld/resizer_pl.ipynb  (FPGA)
```
